...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $i = 1;
        while (true) {
            $s = $pdo->prepare("SELECT id FROM blogs WHERE slug=?");
            $s->execute([$slug]);
            if (!$s->fetch()) break;
            $slug = $slug_base . '-' . (++$i);
        }
    ...
    if (empty($errors)) {
        ...
        flash('success', 'Blog "' . $title . '" created successfully.');
        header('Location: ' . BASE_URL . 'blogs.php');
        exit;
    }
}

// Re-include header after potential redirect
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <textarea name="short_description" class="form-control form-control-sm" rows="3"
                      placeholder="Brief summary for listing pages"><?= htmlspecialchars($_POST['short_description'] ?? '') ?></textarea>
          </div>

          <div class="form-group">
            <label style="font-size:12.5px;font-weight:600;">Full Content</label>
            <textarea name="content" id="blogContent"><?= htmlspecialchars($_POST['content'] ?? '') ?></textarea>
          </div>
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <textarea name="meta_description" class="form-control form-control-sm" rows="3"
                      placeholder="SEO description" maxlength="320"><?= htmlspecialchars($_POST['meta_description'] ?? '') ?></textarea>
